...
router.post('/users', async (req, res, next) => {
  try {
    ...
    if (!email || !name || !password || !passwordConfirm) {
      res.status(400).json('정보를 모두 입력해 주세요');
    }
    // 이메일 중복
    if (existUser) {
      res.status(400).json('이미 가입 된 사용자입니다');
    }
    // 이메일 형식이 맞지 않음
    const val = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!val.test(email)) {
      res.status(400).json('이메일 형식이 올바르지 않습니다.');
    }
    // 비밀번호가 6자리 미만인 경우
    if (password.length < 6) {
      res.status(400).json('비밀번호는 6자리 이상이어야 합니다.');
    }
    // 비밀번호와 비밀번호 확인이 일치하지 않는 경우
    if (password !== passwordConfirm) {
      res.status(400).json('입력한 두 비밀번호가 일치하지 않습니다.');
    }
